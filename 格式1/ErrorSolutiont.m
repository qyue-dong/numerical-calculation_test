function [ U, u,  E ] = ErrorSolutiont( h, ts, q)
%-----------------------------------偏微分方程的空间参数赋值-------------------------------
xl = -80;                                                                    %空间变量左端 
xr = 80;                                                                    %空间变量右端点
L = xr - xl;                                                                 %空间变量x的区间长度
...
X = double(X);
%-----------------------------------双孤子精确解------------------------------------------
% 双孤子精确解：两个孤立波的叠加（忽略相互作用项）
u = A1 * (sech(K1 * (X - (x10) ))).^2 + A2 * (sech(K2 * (X - (x20)))).^2 + A3 * (sech(K3 * (X - (x30)))).^2;
u0=u(:,1);                                    %微分方程在t=0时的解，
